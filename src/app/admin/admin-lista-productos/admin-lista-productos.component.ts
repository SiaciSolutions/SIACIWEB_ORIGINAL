...
@Component({
  ...
  styleUrls: ['./admin-lista-productos.component.css']
})
...
export class AdminListaProductosComponent implements OnInit {
  ...
  // public dtTrigger: Subject<any> = new Subject();
   ...
   ngOnInit() {
	...
	this.srv.lista_productos(datos).subscribe(
	   data => {
		   // if (data){
			   // this.loading = false;
		   // }
		...
		this.dtOptions = {
			...
			buttons: [{
                extend: 'print',
                filename: 'LISTA_PRODUCTOS_SIACI_WEB'
            },
            {
                extend: 'excel',
                filename: 'LISTA_PRODUCTOS_SIACI_WEB'
    ...
    render_table(new_list): void {
		...
		setTimeout(()=> {	
			...
			this.dtOptions = {
				...
				buttons: [{
					extend: 'print',
					filename: 'LISTA_PRODUCTOS_SIACI_WEB'+this.usuario
				},
				{
					extend: 'excel',
					filename: 'LISTA_PRODUCTOS_SIACI_WEB'+this.usuario
   ...
   eliminar_producto(codart): void {
		...
		if (confirm("Esta seguro de Eliminar este producto..???")){
			this.srv.eliminar_articulo(datos).subscribe(
			   data => {
				   console.log (data)
				   ...
				   if (data['STATUS'] == 'ARTICULO_TIENE_MOVIMIENTO'){
					 alert("ERROR..!! Articulo posee movimientos en el sistema. Articulo no puede ser eliminado..!!!!")
