...
@Component({
  ...
  templateUrl: './admin-view-pedido.component.html',
  styleUrls: ['./admin-view-pedido.component.css']
})
...
export class AdminViewPedidosComponent implements OnInit {
  ...
  this.loading = true;
   ...
   ngOnInit() {
	...
	this.route.queryParams.subscribe(params => {
		...
		this.pedido = params['pedido'] || this.route.snapshot.paramMap.get('pedido');
		this.sucursal = params['sucursal'] || this.route.snapshot.paramMap.get('sucursal');
...
// }
	...
	this.srv.get_renglones_pedido(datos).subscribe(
	   data => {
		   console.log(data)
		   console.log ("EJECUTADA DATA")
		   this.renglones_pedido_tabla = data
		...
		setTimeout(()=> {	
			...
			this.renglones_pedido = this.renglones_pedido_tabla
		...
		this.dtOptions = {
			...
			buttons: [{
                extend: 'print',
				text: 'Imprimir',
				title: 'Pedido Num: '+this.pedido+' / Cliente: '+this.cliente+' / Sucursal: '+this.sucursal,
                filename: 'PEDIDO_'+this.pedido,
				exportOptions: {
                    columns: [ 1, 2, 3, 7 ]
                },
