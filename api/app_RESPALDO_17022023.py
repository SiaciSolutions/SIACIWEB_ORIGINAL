...
def get_encabezado_pedido():
	...
	round((p.totnet+iva_cantidad),2) as total_pedido, p.codven, p.codalm
	...
	campos = ['num_pedido', 'fectra','fecult','identificacion','cliente','direccion','codcli','telefono','email','observ','totnet','iva_cantidad','codusu','ciucli','nomven','total_pedido','codven','codalm']
	print (r)
	if r:
...
def get_encabezado_pdv():
    ...
    p.totiva,p.poriva,p.totdes,p.pordes,p.totbas
	...
	,'valefe','tiptar','numtar','valtar','codtar','tipche','numche','valche','codban','tipcre','valcre','numpag','plapag','tiptrans','valtrans','numtrans','coddep','totiva','poriva','totdes','pordes','totbas']
	print (r)
	if r:
		d = dict(zip(campos, r))
...
def eliminar_articulo():
  ...
  print (art)
  if (art[0] == 0):
    sql = """ delete from articulos where codart='{}' and codemp='{}';
    """.format(datos['codemp'],datos['codart'])
    print (sql)
    curs.execute(sql)
    conn.commit()
...
def crear_paciente():
  ...
  ##PARA VER DUPLICADO DE CLIENTE
  sql = "select count(*) from clientes where  codemp= '{}' and rucced='{}' and tpIdCliente='C'"\
        .format(datos['codemp'],datos['rucced'])
  curs = conn.cursor()
  curs.execute(sql)
  r = curs.fetchone()
  ...
  if (exist_cliente == 0 and datos['tpIdCliente'] != 'X'):
      print ("###### CREO PACIENTE  ####")
      sql = """
      ...
      """.format(datos['codemp'],codcli,datos['nomcli'],datos['apecli'],datos['rucced'],datos['codus1'],datos['fectra'],seccli,datos['tpIdCliente'],datos['num_historia'],datos['fecha_nacimiento'])
      print (sql) 
      curs.execute(sql)
      conn.commit()
      d = {'STATUS': 'EXITOSO'}
  elif (datos['tpIdCliente'] == 'X'):
      print ("###### CREO PACIENTE SIN CECULA ####")
      sql = """
      insert into clientes (codemp,codcli,nomcli,apecli,rucced,estatus,apliva,limcre,lispre,codusu,fecult,codven,seccli,tipo,codcla,tpIdCliente,tipovendedor,num_matricula,fecha_nacimiento)
      values ('{}','{}','{}','{}',NULL,'A',0,1,1,'{}',DATE('{}'),'01','{}','P','01','{}','G','{}','{}')
      """.format(datos['codemp'],codcli,datos['nomcli'],datos['apecli'],datos['codus1'],datos['fectra'],seccli,'P',datos['num_historia'],datos['fecha_nacimiento'])
      print (sql) 
      curs.execute(sql)
      conn.commit()
      d = {'STATUS': 'EXITOSO'}
